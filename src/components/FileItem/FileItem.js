    ...
    if (props.state.movingMode) {
        ...
        return (
            <tr className={props.state.editNameMode ? "editable__row" : props._id !== props.selected ? "" : "active__recent"} onClick={props.startFileClick}
                ...
                <td class="expired__row noSelect">{props.expiredAt ? moment(props.expiredAt).format("L") : 'Not specified'}</td>

                <td class={`expired__row noSelect ${props.expiredAt ? ((new Date()).getTime() > (new Date(props.expiredAt)).getTime() ? 'expired__row__active' : '') : ''}`}>
                    {
                        props.expiredAt
                            ? moment(props.expiredAt).format("L")
                            : 'Not specified'
                    }
                </td>
